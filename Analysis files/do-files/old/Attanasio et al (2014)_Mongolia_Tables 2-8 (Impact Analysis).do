...
* ---------------------------------------------- *
* 	Table 8 -- Impacts of individual liability
* ---------------------------------------------- *

	*-- GLOBAL for the variabls presented in the Table:
	global VariablesT8 "dum_loan_x dum_any_loan enterprise soleent ln_totalpc ln_foodc"

	global SimpleSpecDD_I "followup indiv IF"
	
	
	*===== SIMPLE DIFFERENCE ====*
	local table = "replace"
	foreach x of varlist $VariablesT8 { 
		qui reg `x' indiv BL`x' $Xvar aimagd* if group!=1 & followup==1, robust cl(soum)
		...
		outreg2 indiv using "output\Results - Table 8 (I&C only)\table_8_indiv_impacts (simple diff)", cttop("`x'") nocons ///
		sortvar(indiv) noobs ///
		addstat(Number of households, `numberhh', Mean Control Follow, `meanfollow') dec(3) ///
		excel `table' ///
		title("Table 8. Individual Impacts (simple diff).") nor2
		local table = "append"
		}
